...
Janus.DefaultDependencies = function (deps) {
...
Janus.mediaToTracks = function (media) {
  ...
  if (!media) {
    ...
    tracks.push({ type: "video", capture: true, recv: true, zoom: true });
  } else {
    if (
      !media.keepAudio &&
      media.audio !== false &&
...
Janus.init = function (options) {
  ...
  if (Janus.initDone) {
    ...
    let usedDependencies = options.dependencies || Janus.DefaultDependencies();
    console.log(usedDependencies.extension);
    ...
    Janus.attachMediaStream = function (element, stream) {
      console.warn(element, "stream ", stream);
      try {
...
function Janus(gatewayCallbacks) {
  ...
  console.log(Janus.isArray(server));
  ...
  let iceServers = gatewayCallbacks.iceServers || [
    {
      urls: "turn:13.57.235.158:3478",
      credential: "test123",
      username: "test",
    },
  ];
  console.log("gatewayCallbacks", gatewayCallbacks);
  ...
  function handleEvent(json, skipTimeout) {
    ...
    if (json["janus"] === "keepalive") {
      ...
      console.warn('retun from handle event');
      return;
  ...
  function createSession(callbacks) {
    ...
    Janus.httpAPICall(server, {
      ...
      success: function (json) {
        ...
        Janus.sessions[sessionId] = that;
        console.log(Janus.sessions[sessionId]);
        eventHandler();
        callbacks.success();
      },
      error: function (textStatus, errorThrown) {
  ...
  function createHandle(callbacks) {
    ...
    Janus.httpAPICall(server + "/" + sessionId, {
      ...
      body: request,
      success: function (json) {
        console.log("attach plugin res =========== ", json);
        Janus.debug(json);
        if (json["janus"] !== "success") {
          Janus.error(
            "Ooops: " + json["error"].code + " " + json["error"].reason
  ...
  function sendMessage(handleId, callbacks) {
    console.log("sendmessage callbacks ", callbacks, handleId);
    ...
    if (websockets) {
      ...
      transactions[transaction] = function (json) {
        ...
        if (json["janus"] === "success") {
          ...
          let data = plugindata["data"];
          Janus.log(data, 'callbacks ', callbacks);
    ...
    Janus.httpAPICall(server + "/" + sessionId + "/" + handleId, {
      ...
      success: function (json) {
        ...
        console.log("callbacks.success ", callbacks.success);
        // If we got here, the plugin decided to handle the request asynchronously
        callbacks.success();
      },
      error: function (textStatus, errorThrown) {
  ...
  function createPeerconnectionIfNeeded(handleId, callbacks) {
    console.log("createPeerconnectionIfNeeded");
    let pluginHandle = pluginHandles[handleId];
    if (!pluginHandle || !pluginHandle.webrtcStuff) {
      Janus.warn("Invalid handle");
    ...
    console.log("config.pc ", config.pc);
        ...
        if (!trackMutedTimeoutId) {
          trackMutedTimeoutId = setTimeout(function () {
            ...
            let transceiver = config.pc
              .getTransceivers()
              .find((t) => t.receiver.track === ev.target);
            let mid = transceiver ? transceiver.mid : ev.target.id;
            try {
              pluginHandle.onremotetrack(ev.target, mid, false);
            } catch (e) {
  ...
  async function prepareWebrtc(handleId, offer, callbacks) {
    console.log("prepareWebrtc ", callbacks);
    callbacks = callbacks || {};
    callbacks.success =
      typeof callbacks.success == "function" ? callbacks.success : Janus.noop;
    callbacks.error =
    ...
    if (callbacks.media && !callbacks.tracks) {
      ...
      console.log("tracks ", callbacks.tracks);
      if (
    ...
    try {
      ...
      if (!jsep) {
        ...
        Janus.log("sdp ", JSON.stringify(jsep));
        console.log("config", config.pc);
  ...
  function prepareWebrtcPeer(handleId, callbacks) {
    console.warn("callbacks ", callbacks);
    callbacks = callbacks || {};
    callbacks.success =
      typeof callbacks.success == "function" ? callbacks.success : Janus.noop;
    callbacks.error =
  ...
  async function captureDevices(handleId, callbacks) {
    ...
    let config = pluginHandle.webrtcStuff;
    console.log("captureDevices config ", config);
    ...
    console.error("callbacks.jsep ", callbacks.jsep);
        ...
        if (track.replace) {
          ...
          if (track.transforms) {
            if (sender && track.transforms.sender) {
              ...
              if (senderStreams) {
                console.log(
                  "Insertable Streams sender transform:",
                  senderStreams
                );
                if (
            ...
            if (
              ...
              if (receiverStreams) {
                console.log(
                  "Insertable Streams receiver transform:",
                  receiverStreams
                );
                if (
